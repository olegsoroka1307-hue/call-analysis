...
|---|---|---|
| **§3 — Gate 0** | валідація data providers на 20–30 матчах | 🔴 **не пройдений — немає API-ключів** ([звіт](gate0/GATE0_REPORT.md)) |
| **§52 — PoC на 1 день** | наскрізний ланцюг odds → EV → рішення | 🟢 **готовий, працює** |

**Технічний аудит 29.08.2026 — CONDITIONAL.** Математичне ядро прийняте,
10 пунктів на виправлення закриті. Що саме було не так і що змінилось —
[docs/AUDIT-2026-08-29.md](docs/AUDIT-2026-08-29.md) і розділ 9 нижче.
...
# 2. Залежності — точні версії, на яких прогін зелений
...
```bash
docker compose down -v      # чистий volume
docker compose up --build
curl http://127.0.0.1:8000/health
```

Compose піднімає **backend + postgres**: міграції, генерація replay-датасету,
прогін PoC і uvicorn. У backend є healthcheck, тому `docker compose ps`
показує реальну готовність, а не просто «контейнер живий».

**Межа відповідності §47.** ТЗ §47 описує стек ширше, ніж ці два сервіси:
Redis, scheduler (§27), Telegram-воркер (§29) і фронтенд (§30–§34) сюди не
входять — вони належать пізнішим спринтам, і зараз їх нема чого запускати.
Тому §47 закритий **частково**, і критерій приймання «запускається через
Docker Compose» треба читати як «PoC-контур запускається», а не «увесь стек».
...
1. **Матчі синтетичні (§52 п.2)** — прямий наслідок незакритого Gate 0.
2. **§47 закритий частково** — Compose містить лише backend і postgres,
   решта сервісів належить пізнішим спринтам (див. розділ 2).
3. **TimescaleDB не використана (§4)** — для 5 матчів звичайного PostgreSQL
...
│   ├── migrations/              # alembic: append-only тригер, provenance, namespace
│   ├── tests/                   # unit + інтеграційні в справжньому PostgreSQL (§46)
│   ├── requirements.lock        # точні версії — Docker і CI ставлять саме їх
...
├── docs/
│   ├── formulas.md              # §14, §16 — виведення формул
│   └── AUDIT-2026-08-29.md      # технічний аудит і що по ньому зроблено
└── docker-compose.yml           # §47, частково (backend + postgres)
```
